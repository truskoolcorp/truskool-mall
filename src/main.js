// main.js — Mall + Arch + Glow + camera polish
// CDN modules (no build step)
...
controls.minDistance = 3.4;   // tighter zoom range
controls.maxDistance = 7.6;
controls.minPolarAngle = Math.PI * 0.18;  // keep horizon in frame
controls.maxPolarAngle = Math.PI * 0.48;
// Optional: narrow side-to-side rotation a touch (uncomment to lock)
...
// ---------- Try to load GLB arch ----------
let archTemplate = null;
try {
  const glb = await new GLTFLoader().loadAsync('/assets/models/portal.glb');
  archTemplate = glb.scene || glb.scenes?.[0] || null;
  if (archTemplate) {
    const box = new THREE.Box3().setFromObject(archTemplate);
    const size = box.getSize(new THREE.Vector3()).length() || 1;
    archTemplate.scale.setScalar(2.2 / size); // normalize width
  }
} catch (e) {
...
function makeGlowSprite() {
  ...
  sp.scale.set(2.6, 2.6, 1);           // base radius
  sp.userData.baseScale = 2.6;
  return sp;
}
...
const hoverMat = new THREE.MeshStandardMaterial({
  emissive: 0x5a6bff, emissiveIntensity: 0.35, metalness: 0.1, roughness: 0.6
});
...
function createFlatCard(store, pos, lookAt) {
  ...
  label.position.copy(pos).add(new THREE.Vector3(0, 0.95, 0)); scene.add(label);
...
function createPortal(store, index, total) {
  ...
  // Glow sprite (sits slightly behind the card)
...
// ---------- Interactions ----------
let hovered = null;
function pick(x,y){ mouse.x=(x/window.innerWidth)*2-1; mouse.y=-(y/window.innerHeight)*2+1;
  raycaster.setFromCamera(mouse,camera); const hit=raycaster.intersectObjects(clickable,false)[0]; return hit?hit.object:null; }
