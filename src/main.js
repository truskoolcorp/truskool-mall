/* Virtual Tru Skool Mall (Turnkey)
   ...
   - Brighter showroom lighting
   - Clickable brand signs & top chips
   - Portal GLB loader (optional, if assets/models/portal.glb exists)
*/
...
// ---------- Load Three from local vendor/, else CDN ----------
async function loadThree() {
  ...
  try {
    ...
    return { THREE, OrbitControls, GLTFLoader, source:'local' };
  } catch (e) { last = e; }

  // fall back to public CDNs (two tries)
  const bases = ['https://unpkg.com/three@0.159.0', 'https://cdn.jsdelivr.net/npm/three@0.159.0'];
  for (const base of bases) {
    try {
      ...
      return { THREE, OrbitControls, GLTFLoader, source:base };
    } catch (e) { last = e; }
  }
...
const { THREE, OrbitControls, GLTFLoader } = await loadThree().catch(err => {
  console.error(err);
...
renderer.toneMappingExposure = 1.35; // brighter
...
const angleStep = Math.min(rad(30), (Math.PI * 1.4) / Math.max(3, STORES.length));
...
for (let i=0;i<STORES.length;i++){
  ...
  const crossGeo = new THREE.BoxGeometry(1.0, 0.12, 0.12);
  const cross = new THREE.Mesh(crossGeo, postMat); cross.position.set(0, 1.5, 0);

  const frame = new THREE.Group();
  frame.add(postL, postR, cross);
  ...
  face.position.set(0, 1.1, -0.07);
  face.castShadow = true;
  face.receiveShadow = true;

  // name label
  const label = makeLabelSprite(s.name, '#ffffff');
  label.position.set(0, 1.9, 0);

  const sign = new THREE.Group();
  sign.add(frame, face, label);
...
try {
  ...
  console.log('Portal GLB not found/loaded. Skipping.', e?.message || e);
}
...
function onClick(ev){
  ...
  if (hits.length) {
    ...
    const g = obj.parent?.parent || obj.parent;
    const link = g?.userData?.link;
